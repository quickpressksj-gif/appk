...
export function LiveDeliveryMap({
  ...
  useEffect(() => {
    ...
    const toLoc = destinationLocation || storeLocation;
    if (fromLoc && toLoc && fromLoc !== toLoc) {
      const dist = getDistanceKm(fromLoc.lat, fromLoc.lng, toLoc.lat, toLoc.lng);
      setDistanceKm(dist);
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      if (!mapInstanceRef.current) {
        const initialCenter = storeLocation || riderLocation || destinationLocation || defaultCenter;
      ...
      if (riderLocation && riderLocation.lat && riderLocation.lng) {
        ...
        if (markersRef.current.rider) {
          ...
          markersRef.current.rider.bindPopup(
            `<b>${riderLocation.label || "Assigned Captain"}</b><br/>Live Position`
          );
        }
      }

      // 2. Store Marker (Partner Store)
      if (storeLocation && storeLocation.lat && storeLocation.lng) {
        ...
        if (markersRef.current.store) {
          ...
          markersRef.current.store.bindPopup(
            `<b>${storeLocation.label || "Your Laundry Store"}</b><br/>Pickup Hub`
          );
        }
      }
      ...
      if (destinationLocation && destinationLocation.lat && destinationLocation.lng) {
        ...
        if (markersRef.current.dest) {
          ...
          markersRef.current.dest.bindPopup(
            `<b>${destinationLocation.label || "Customer Destination"}</b>`
          );
        }
      }

      // 4. Route Polyline
      const p1 = riderLocation || storeLocation;
      const p2 = destinationLocation || storeLocation;
      if (p1 && p2 && p1 !== p2) {
        const routePoints = [
          [p1.lat, p1.lng],
          [p2.lat, p2.lng],
  ...
  const handleRecenter = () => {
    ...
    const focus = storeLocation || riderLocation || defaultCenter;
  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm select-none ${heightClassName}`}>
      ...
      <div className="absolute top-3 left-3 z-10 flex flex-wrap items-center gap-1.5">
        <div className="flex items-center gap-2 rounded-xl bg-white/95 backdrop-blur-md px-3 py-1.5 shadow-md border border-slate-200 text-xs">
          ...
          <span className="font-bold text-slate-800">
            {phase === "pickup" ? "Captain Arriving" : "Captain Delivering"}
          </span>
          {distanceKm !== null ? (
            <>
              <span className="text-slate-300">·</span>
              <span className="font-black text-indigo-700">{distanceKm} km</span>
            </>
          ) : null}
          {etaMins !== null ? (
            <>
      ...
      {showControls ? (
        <div className="absolute top-3 right-3 z-10 flex flex-col gap-1.5">
          <button
            ...
            title="Recenter"
          >
            <Crosshair className="size-4 text-indigo-700" />
          </button>
          <button
            type="button"
