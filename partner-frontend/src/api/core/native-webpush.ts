/**
 * QuickPress Partner — Pure Native WebPush (VAPID / RFC 8292) Client.
 */
...
export async function fetchVapidPublicKey(): Promise<string | null> {
  try {
    ...
    console.debug("[Partner-WebPush] Failed to fetch VAPID public key:", err);
    return null;
  }
}
...
export async function subscribeToNativeWebPush(): Promise<PushSubscription | null> {
  ...
  try {
    ...
    console.debug("[Partner-WebPush] Subscription error:", err);
  }
  return null;
}
...
export async function unsubscribeFromNativeWebPush(): Promise<boolean> {
  ...
  try {
    ...
    console.debug("[Partner-WebPush] Unsubscribe error:", err);
  }
  return false;
}
...
export async function triggerTestWebPush(): Promise<boolean> {
  try {
    const res = await apiPostJson<{ ok: boolean; sentCount: number }>(
      ...
      {
        title: "⚡ Partner Store Push Working!",
        body: "Order notifications will now appear directly on your desktop/browser screen.",
        url: "/orders",
      }
    );
    if (res?.ok) {
      toast.success("Native partner push dispatched!");
      return true;
    }
  } catch (err) {
