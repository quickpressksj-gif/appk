...
export function RiderSuspendedScreen() {
  ...
  const { session, signIn, signOut } = useRiderContext();
  ...
  const fetchSuspensionDetails = async (silent = false) => {
    ...
    try {
      ...
      if (res.status === "active" || res.status === "approved") {
        ...
        toast.success("🎉 Your Captain account is now ACTIVE! Redirecting to Dashboard...");
        setTimeout(() => navigate({ to: riderRoutes.dashboard }), 600);
      }
    } catch {
      // Fallback
    } finally {
  ...
  const handleAppealSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiPostJson("/api/rider/appeal", { reason: appealText });
      toast.success("Appeal submitted successfully! Trust & Safety team has been notified.");
      setAppealText("");
  ...
  const handleCopyId = () => {
    if (typeof navigator !== "undefined" && navigator.clipboard) {
      ...
      toast.success("Captain ID copied to clipboard!");
    }
  };

  const handleWhatsApp = () => {
    const text = encodeURIComponent(
      `Hello QuickPress Trust & Safety Team, my Captain ID is: ${captainId}. My account is suspended and I would like assistance regarding my appeal.`
    );
        ...
        <div>
          ...
          <div className="animate-slide-up text-center">
            ...
            <h1 className="mt-3 text-2xl font-black tracking-tight text-white sm:text-3xl">
              Captain Account Suspended
            </h1>
            <p className="mt-2 text-xs font-medium leading-relaxed text-slate-400 max-w-sm mx-auto">
              Your QuickPress Captain delivery dispatch privileges and live order access have been locked by Operations Trust &amp; Safety.
            </p>
          </div>

          {/* Captain ID & Suspension Reason Card */}
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="rounded-2xl border border-rose-900/50 bg-rose-950/30 p-3.5 text-xs font-semibold text-rose-200 leading-snug">
                {profileData?.suspensionReason ||
                  "Account under review due to policy compliance or quality standards check."}
              </div>
            </div>
            ...
            {isAppealPending ? (
              <div className="text-center py-2">
                ...
                <p className="mt-1.5 text-xs font-medium text-slate-400 leading-relaxed max-w-md mx-auto">
                  Your appeal has been recorded and submitted to the Trust &amp; Safety team. We review appeals within 24 hours. Your account remains inactive until explicitly approved.
                </p>
              ...
              <form onSubmit={handleAppealSubmit}>
                ...
                <p className="mt-1 text-xs text-slate-400">
                  If you believe this suspension is in error or you have resolved the issue, submit an explanation below for reconsideration.
                </p>
                ...
                <textarea
                  ...
                  placeholder="Explain your situation clearly (e.g. why this occurred or what steps you have taken)..."
