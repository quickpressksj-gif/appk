...
export function RiderDashboardScreen() {
    ...
    try {
      ...
      if (backendActive && !activeOrder) {
        setActiveOrder({
          ...
          store_name: backendActive.store_name || "QuickPress Laundry Store",
          ...
          delivery_address: backendActive.delivery_address || "Kasganj City",
          status: backendActive.status || "assigned",
          delivery_fee: backendActive.delivery_fee || 55,
  ...
  const handleAcceptOffer = async (offer: IncomingOffer) => {
    ...
    toast.success("Trip Accepted! Proceed to pickup store.");
  };
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-4xl space-y-4 p-4 sm:p-6 select-none">
        {/* ========================================================================= */}
        {/* 1. BIKE DUTY BANNER (One-Thumb Toggle)                                    */}
        {/* ========================================================================= */}
        <BikeDutyBanner
          isOnline={isOnline}
          onToggle={setOnline}
        ...
        {/* ========================================================================= */}
        {/* 3. ACTIVE ORDER COCKPIT (The Bike Rider's Primary Work Screen)             */}
        {/* ========================================================================= */}
        {activeOrder ? (
        ...
        {/* ========================================================================= */}
        {/* 4. REVENUE & TRIPS HERO CARD (High-Contrast for Bike Sunlight Readability) */}
        {/* ========================================================================= */}
        <div className="rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-sm">
          ...
          {/* 4 Large Metric Columns */}
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
            <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100">
              <p className="text-[10px] font-bold uppercase text-slate-400">Trips Done</p>
              <p className="mt-0.5 text-xl font-black text-slate-900">{completedToday}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100">
              <p className="text-[10px] font-bold uppercase text-slate-400">Active Duty</p>
              <p className="mt-0.5 text-xl font-black text-emerald-700">4.2 hrs</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100">
            ...
            <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100">
              ...
              <p className="mt-0.5 text-xl font-black text-amber-500">4.9 ★</p>
            </div>
          </div>
        </div>
        ...
        {isOnline && !activeOrder ? (
          <div className="relative overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-b from-emerald-50/40 via-white to-white p-7 text-center shadow-sm">
            <div className="relative mx-auto my-3 flex size-28 items-center justify-center">
              <div
                className="absolute size-28 rounded-full border border-emerald-400/30 bg-emerald-400/10 animate-ping"
                style={{ animationDuration: "2.4s" }}
              />
              <div
                className="absolute size-20 rounded-full border border-emerald-400/50 bg-emerald-400/15 animate-ping"
                style={{ animationDuration: "1.6s" }}
              />
              <div className="relative flex size-12 items-center justify-center rounded-2xl bg-emerald-600 text-white shadow-md">
                <Navigation className="size-6 animate-pulse" />
              </div>
            </div>

            <div className="mt-1">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-xs font-black uppercase text-emerald-800">
                <span className="size-2 rounded-full bg-emerald-500 animate-ping" />
                <span>GPS Radar Scanning</span>
              </span>
              <h3 className="mt-2 text-base font-black text-slate-950">
                Waiting for Next Laundry Delivery...
              </h3>
              <p className="mt-0.5 text-xs text-slate-500 max-w-sm mx-auto">
                Scanning Kasganj stores within 5 km. Keep phone on bike mount.
              </p>

              {/* Demo test button to test siren and pickup flow */}
              <div className="mt-4">
                <button
                  type="button"
                  onClick={handleTriggerDemoOrder}
                  className="inline-flex items-center gap-2 rounded-xl bg-slate-900 hover:bg-black text-white px-4 py-2.5 text-xs font-bold shadow-xs active:scale-95 transition-all cursor-pointer"
                >
        ...
        {/* ========================================================================= */}
        {/* 6. QUICK OPERATIONS GRID (Large 54px Tap Targets for Bike)                 */}
        {/* ========================================================================= */}
        <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <button
              ...
              className="flex flex-col items-center gap-2 rounded-2xl border border-slate-100 bg-slate-50 p-4 hover:bg-slate-100 active:scale-95 transition-all cursor-pointer"
            >
              <div className="flex size-11 items-center justify-center rounded-xl bg-slate-900 text-white shadow-xs">
            ...
            <button
              ...
              className="flex flex-col items-center gap-2 rounded-2xl border border-rose-100 bg-rose-50 p-4 hover:bg-rose-100 active:scale-95 transition-all cursor-pointer"
            >
              <div className="flex size-11 items-center justify-center rounded-xl bg-rose-600 text-white shadow-xs">
